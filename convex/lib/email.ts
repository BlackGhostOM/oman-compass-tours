...
export function layout(locale: Locale, title: string, body: string, footerNote?: string, head = "", maxWidth = 560, scale = 1): string {
  const big = (n: number) => Math.round(n * scale);
  const mid = (n: number) => Math.round(n * (1 + (scale - 1) / 2));
...
<tr><td style="padding:28px 32px 8px;text-align:center;letter-spacing:.14em;font-size:${mid(13)}px;color:${BRAND.gold};font-family:Cinzel,Georgia,serif">OMAN COMPASS TOURS</td></tr>
...
<tr><td style="padding:20px 32px 0;text-align:${align};font-size:${big(20)}px;font-weight:600;line-height:1.4">${title}</td></tr>
<tr><td dir="${dir}" style="padding:12px 32px 28px;text-align:${align};font-size:${big(15)}px;line-height:1.7"><div dir="${dir}">${body}</div></td></tr>
<tr><td style="padding:16px 32px 24px;text-align:${align};font-size:${mid(12)}px;line-height:1.6;color:${BRAND.ink300};border-top:1px solid ${BRAND.navy800}">
